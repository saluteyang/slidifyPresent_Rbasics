...
plot(fcast.test, main="DSHW forecast for 2012 August hourly prices")
...
test.train<-ts(test.train,start=1,deltat=1)
fcast.test<-dshw(test.train, period1=24,period2=24*7)
plot(fcast.test,main="DSHW forecast for 2013 August hourly prices")
lines(test,col="black")
```
...
```{r motionchart, results='asis',echo=F}
BB.motion<-gvisMotionChart(x_chart,idvar="path",timevar="time",
                    xvar="time",yvar="price",options = list(width = 500, height = 350))
plot(BB.motion,"chart")
```

```{r ComboExample, results='asis',tidy=TRUE,echo=FALSE}
## Add the mean
